...
void Graphics::DrawCircle(int x, int y, int r, Color c)
{
	...
	/*float step = 360 / (r * 3.14 * 4); // step density scaled to radius - so we don't draw 360 pixels for a radius 3 circle
	for (float i = 0; i < 360; i+= step) { 

		float xPix = x + r * cos(i);
		float yPix = y + r * sin(i);
		

		PutPixel(round(xPix), round(yPix), c);
